...
MIN_BEARISH_EXIT_PROFIT_PCT = float(os.getenv("MIN_BEARISH_EXIT_PROFIT_PCT", "1.0"))
...
def build_macd_signals(df: pd.DataFrame) -> List[dict]:
    ...
    for i in range(2, len(df)):
        ...
        #     max_profit_pct < EARLY_FAIL_MAX_PROFIT_PCT
        # )

        trailing_exit = False
        # trailing_exit = bool(
        #     max_profit_pct >= MIN_TRAIL_PROFIT_PCT and
        #     live_profit_pct > 0 and
        #     live_profit_pct <= (max_profit_pct * TRAIL_GIVEBACK_RATIO)
        # )
